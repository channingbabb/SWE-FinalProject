...
public class GamePanel extends JPanel {
    ...
    protected void paintComponent(Graphics g) {
        ...
        if (players != null && !players.isEmpty()) {
            ...
            displayRiver(g2d, riverX, riverY, dealer.getCommunityCards());
        } else {
    ...
    public void displayRiver(Graphics2D g2d, int x, int y, ArrayList<CardClass> communityCards){
        if (communityCards == null || communityCards.isEmpty()){
            g2d.setColor(Color.white);
            g2d.drawString("No river card to display", x, y);
            System.out.println("No river card to display");
            try {
                cardBackImage = ImageIO.read(new File("assets/card-back.png"));
                g2d.drawImage(cardBackImage, 550, 300, 60, 80, null);
            } catch (IOException e) {
                System.err.println("Error loading card back image: " + e.getMessage());
            }
        }
        ...
        for (int i = 0; i < communityCards.size(); i++){
            ...
            String rank = convertRankToText(card.getRank());
            String imagePath = "assets/cards" + rank + "_of_" + card.getSuit().toLowerCase() + ".png";

            try {
                BufferedImage cardImage = ImageIO.read(new File(imagePath));
                g2d.drawImage(cardImage, x +i * (cardW + cardSpace), y, cardW, cardH, null);
            } catch (IOException e){
                System.out.println("Error loading river card image: " + imagePath);
                g2d.setColor(Color.WHITE);
                g2d.fillRect(x + i * (cardW + cardSpace), y, cardW, cardH);
                g2d.setColor(Color.BLACK);
                g2d.drawString(rank + " of " + card.getSuit(), x + i * (cardW + cardSpace) + 5, y + 20);
            }
        }
    }
